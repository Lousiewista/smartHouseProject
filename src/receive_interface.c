...
static void *handle_device(void *arg) //
{
    ...
    if (NULL != cur_gdev) {
        ...
        if (1 == cur_gdev->voice_set_status) { //语音控制
...
static void *receive_get(void *arg) // 接收消息
{
    recv_msg_t *recv_msg = NULL;
    ssize_t read_len = -1;
    pthread_t tid = -1;
    ...
    if (NULL != arg) { // 获取到mqd 和phead (struct control 链表的头结点)
    ...
    if (mq_getattr(recv_msg->ctrl_info->mqd, &attr) == -1) { // 获取消息队列的属性
        pthread_exit(0);
    }

    recv_msg->buffer = (unsigned char *) malloc(attr.mq_msgsize); // 分配空间
    buffer = (unsigned char *) malloc(attr.mq_msgsize); // 分配空间
    memset(recv_msg->buffer, 0, attr.mq_msgsize); // 初始化
    memset(buffer, 0, attr.mq_msgsize); // 初始化
    pthread_detach(pthread_self()); // 分离线程
    struct timespec timeout = { .tv_sec = 5, .tv_nsec = 0 }; // 设置超时时间

    while (1) {
        // read_len = mq_receive(recv_msg->ctrl_info->mqd, buffer, attr.mq_msgsize, NULL);
        read_len = mq_timedreceive(recv_msg->ctrl_info->mqd, buffer, attr.mq_msgsize, NULL, &timeout); // 接收消息
